...
def set_material(idx, params_current_shader, current_object, working_dir):
    ...
    for key, value in params_current_shader.items():
        ...
        if key.lower() in ["color_texture", "metallic_texture", "specular_texture", "roughness_texture", "sheen_texture",
            ...
            set_texture_mapping(idx=idx, key=key, value=value,
...
def create_light_directions(params_light, working_dir):
    ...
    if params_light['model'].lower() == 'near':
        if axis2bool(params_light['enable_rendering_by_degree']):
            degree = float(params_light['degree'])
            light_directions_euler = np.zeros((int(360 / degree), 5))
            radius = float(params_light['POINT_radius'])
            for i in range(0, int(360 / degree)):
                if params_light['enable_rendering_by_degree'].lower() in ["x"]: axis = 0
...
def create_normal_map(filepath, npy=False):
    ...
    if npy:
        ...
        ix = bpy.data.scenes['Scene'].render.resolution_x
        iy = bpy.data.scenes['Scene'].render.resolution_y
        normal = np.reshape(arr, (iy, ix, 3))
        normal = np.flipud(normal)
        np.save(os.path.join(filepath, 'normals.npy'), normal)
        normal = 2 * (normal - 0.5)
        np.save(os.path.join(filepath, 'normals_alt.npy'), normal)
...
def create_depth_map(filepath, npy=False):
    ...
    # Depth of Render Layers to Normalize
    links.new(rl.outputs[2], normalize.inputs[0])
    links.new(normalize.outputs[0], comp_node.inputs[0])  # invert to Composite
    links.new(rl.outputs[2], viewer_node.inputs[0])  # invert to Viewer

    # render
    bpy.ops.render.render()
    bpy.data.images['Render Result'].save_render(
        filepath=os.path.join(filepath, 'depth.png'))

    # save real depth as array
    pixels = bpy.data.images['Viewer Node'].pixels
    pixels = np.array(pixels)[::4]
    ix = bpy.data.scenes['Scene'].render.resolution_x
    iy = bpy.data.scenes['Scene'].render.resolution_y
    distance = np.reshape(pixels, (iy, ix))
    distance = np.flipud(distance)
    np.save(os.path.join(filepath, 'distance.npy'), distance)
    mask = np.where(distance != 1e+10, 1, 0)
    np.save(os.path.join(filepath, 'mask.npy'), mask)

    if os.path.exists(os.path.join(filepath, 'normals_alt.npy')):
        N = np.load(os.path.join(filepath, 'normals_alt.npy'))
        for h in range(iy):
            for w in range(ix):
                if np.linalg.norm(N[h][w]) < 0.8:
                    mask[h][w] = 0
        np.save(os.path.join(filepath, 'mask_alt.npy'), mask)

    if npy:
        tmp = np.where(distance != 1e+10, distance, 0)
        max_d, min_d = np.amax(tmp), np.amin(distance)
        distance = np.where(distance != 1e+10, distance, max_d)
        depth = (distance - min_d) / (max_d - min_d)
        np.save(os.path.join(filepath, 'depth.npy'), depth)
...
def render_object(object_shape, params_rendering, params_camera, params_object_global,
                  params_object, params_shader, params_light, working_dir, out_dir, *args, **kwargs):
    ...
    for material in bpy.data.materials:
        if 'Default OBJ' in material.name or '(null)' in material.name:
    ...
    out_dir = os.path.join(os.path.abspath(out_dir), obj_name)
    ...
    for index, label in enumerate(object_directions_euler):
        ...
        for i, l in enumerate(light_directions_euler):
            ...
            if str2bool(params_object_global['enable_multiple_views']) and str2bool(params_light['synchronize_with_views']):
                for item in bpy.data.objects:
                ...
                bpy.ops.transform.rotate(value=-label[1], orient_axis='Y')
                bpy.ops.transform.rotate(value=-label[2], orient_axis='Z')
            if str2bool(params_object_global['enable_multiple_views']) and (use_hdri) and str2bool(params_rendering['synchronize_with_views']):
                mapping_node = bpy.data.worlds['World'].node_tree.nodes["Mapping"]
                mapping_node.inputs[2].default_value[2] = mapping_node.inputs[2].default_value[2] - label[1]
                mapping_node.inputs[2].default_value[1] = mapping_node.inputs[2].default_value[1] - label[2]
                mapping_node.inputs[2].default_value[0] = mapping_node.inputs[2].default_value[0] - label[0]
                bpy.context.view_layer.update()
...
for arg in arglist:
    ...
    with open(os.path.join(ini['settings']['out_dir'], obj_name, 'save.ini'), 'w') as f:
